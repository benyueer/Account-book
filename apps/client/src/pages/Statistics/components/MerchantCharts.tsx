...
export const MerchantCharts = ({ data }: Props) => {
  ...
  return (
    <Card title="商户支出排行" className="rounded-2xl border-none shadow-sm mb-6">
      <div className="mb-4">
        <Segmented
          options={[
        ...
        {filteredData.length > 0 ? (
          <>
                ...
                <Tooltip
                  ...
                  formatter={(value: any, _name: any, props: any) => {
                    ...
                    return [
                      ...
                      <span key="name" className="text-slate-500 text-xs">商户: {item?.name}</span>
                    ]
                  }}
                />
              </PieChart>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="text-xs text-slate-400">总{type === 'expense' ? '支出' : '收入'}</span>
        ...
        {filteredData.slice(0, 5).map((item, index) => (
          <div key={item.name} className="flex items-center justify-between text-sm">
            ...
            <div className="flex items-center gap-4">
              <span className="text-slate-300 text-xs">{item.percentage.toFixed(1)}%</span>
              <span className="text-slate-700 font-semibold min-w-[80px] text-right">{formatAmount(item.amount)}</span>
            </div>
          </div>
        ))}
      </div>
